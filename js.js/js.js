...
$.ajax(url)
    ...
    .done(function(xml){
            $(xml).find("pirata").each(function(){
                ...
                if(id == pos){
                $("#quemé.html").append('<div class="card"><p class="procurado">Procurado</p> <img class="foto"src="'+ $(this).find("id").text() +'.jpg"><p class="Rap line">Rap line ou Vocal line</p> <p class="nome">'+ $(this).find("nome").text() +'</p> <p>'+ $(this).find("alcunha").text()+'<\p> <p class="rec"> <img class="Logo-BTS-PNG.png" src="Logo-BTS-PNG.png">'+ $(this).find("recompensa").text()+' </p><p> Status:'+ $(this).find("status").text()+' </p><\div>');
                }
            });
